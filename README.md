...
- 🌗 **深浅色外观**：默认跟随系统，顶栏外观按钮循环「跟随系统 → 浅色 → 深色」并记住选择；首帧前定主题，不会闪白
- 🧩 **浮层与布局记忆**：右侧抽屉同一时刻只开一个（后开的替换前一个），`Esc` 关闭最上层抽屉/弹窗；视图模式（编辑/分栏/预览）、侧边栏折叠、分栏宽度、目录开关刷新后保持原样
- 💾 **保存状态**：状态栏右侧常驻「未保存 / 保存中… / 已保存 时刻」（落库失败显示「未保存（离线）」）
- 🔗 **编辑 / 预览滚动联动**：分栏下两侧按标题锚点双向同步滚动
- ⌨️ **快捷键**：`Ctrl/Cmd+K`（或 `P`）全文搜索、`Ctrl/Cmd+B` 折叠/展开侧边栏、`Ctrl/Cmd+S` 立即保存、`Esc` 关闭最上层抽屉或弹窗
- 🪟 **应用内对话框**：所有原生 `confirm()` / `prompt()` / `alert()`（新建、重命名、删除、改密码、接受智能体改动…）都换成应用内对话框 —— 危险操作红色按钮、多行文案能排版、深色主题下不闪白（原生框是系统白底）、移动端 Safari 也能用；密码框不回显，`Esc` / 点遮罩 / 取消均为取消，关闭后焦点回到原按钮
- 🔔 **提醒浮层**：失败类提示除了状态栏那行，另外弹一个 4 秒自动消失的 toast（`aria-live` 读屏可念）
- 🔑 **静态 API Token**：设置页可为当前用户设置 16～64 位字母数字 Token（服务端只存 sha256，不存明文），之后调任何需登录接口都可用 `Authorization: Bearer <token>`，与登录 Cookie 等价；清空后立即失效。适合脚本 / 智能体 / CI 拉文档：
